...
export default function Lanyard({
    ...
    return (
        <div ref={containerRef} className="relative z-0 w-full h-[50vh] md:h-[480px] lg:h-screen flex justify-center items-center transform scale-100 origin-center touch-pan-y">
            <Canvas
                camera={{ position, fov }}
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, isInView = true }: BandProps) {
    ...
    const innerCard = useRef<any>(null); // Ref baru untuk grup mesh visual kartu
    ...
    useFrame((state, delta) => {
        ...
        // Animasi membalik kartu menggunakan lerp
        if (innerCard.current) {
            innerCard.current.rotation.y = THREE.MathUtils.lerp(
                innerCard.current.rotation.y,
                isFlipped ? Math.PI : 0,
                delta * 10
            );
        }
